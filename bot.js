...
client.on("message", message => {
    const dmchannel = client.channels.find("name", "🔗・bot-log");
...
let limit = "3" // 3 kısmına ban limitinin kaç olmasını istiyorsanız yazınız.
  if(banlar >= limit) {
guild.member.kick(user,{reason: "AKASYA, Atıldınız. (Ban limitinizi aştınız.)"})    
...
.setTimestamp()
 ...
 if (m.channel.id !== "Görsel Kanal ID") { // Buraya o kanalın ID'si yazılacaktır.
    return;
  }
  if (m.author.id === m.guild.ownerID) return;
